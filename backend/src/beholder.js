...
module.exports = class Beholder {
    ...
    constructor(automations, testCache = false) {
        ...
        setTimeout(() => {
            ...
            logger('beholder', `Beholder Brain has started for user ${automations[0].userId}!`);
        }, 1000)
    }
    ...
    async calcQty(orderTemplate, price, symbol, position = null) {
        ...
        if (/^(MAX_F?WALLET)$/.test(orderTemplate.quantity)) {
            ...
            newQty = Math.abs(parseFloat(position.positionAmt)) * multiplier;//Math.abs vai garantir que seja positivo mesmo em caso de SHORT
        }
