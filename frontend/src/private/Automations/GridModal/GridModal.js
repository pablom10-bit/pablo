...
function GridModal(props) {
    ...
    useEffect(() => {
        ...
        if (!props.data.id || !props.data.grids || !props.data.grids.length || !props.data.conditions) return setGrid({ ...DEFAULT_GRID });
    ...
    function onGridChange(event) {
        const value = event.target.value === 'Min. Notional' ? 'MIN_NOTIONAL' : event.target.value.replace(',', '.');
        grid[event.target.id] = value;

        setGrid(prevState => ({ ...prevState, [event.target.id]: value }));

        if (event.target.id === 'quantity' && parseFloat(grid.quantity) < parseFloat(symbol.minLotSize)) {
                                            ...
                                            <div className="row">
                                                <div className="col-md-6 mb-3">
                                                    <div className="form-group">
                                                        ...
                                                        <input className="form-control" id="levels" type="number" placeholder="3" value={grid.levels || ''} disabled={automation.id > 0} onChange={onGridChange} />
                                                    </div>
                                                </div>
                                                <div className="col-md-6 mb-3">
                                                    <div className="form-group">
